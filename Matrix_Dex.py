...
def Matrix():
    ...
    kills = losses = shards = numBattles = turns = parasite = medicSkill = maelstrom = fireball = terrify = 0
            ...
            while(MI.locateCenterOnScreen('images/skills/playerbubble.png', confidence = 0.70, region = (1098, 254, 27, 20))):
                ...
                if(MI.locateCenterOnScreen('images/skills/strike.png', confidence = 0.70,region = (1391,535,33,46))):
                    ...
                    ##if not (MI.locateCenterOnScreen('images/skills/matrix_low.png', confidence = 0.95, region = (1720, 240, 60, 70))):
                        ##killQuick(parasite, medicSkill, maelstrom, MI.locateCenterOnScreen('images/skills/fireball.png', confidence = 0.45,region = (1020,665,680,90)), terrify)

                    ##Fireball
                    ##MI.click(1050,700)
                    MI.click(MI.locateCenterOnScreen('images/skills/fireball.png', confidence = 0.45,region = (1020,665,680,90)))
            ...
            if battleOver:
                if(MI.locateCenterOnScreen('images/didYouKnow.png',confidence = 0.7, region = (1019, 752, 53, 49))):
                    ...
                    if PMFlag:
                        ...
                        return
                    if(kills == killLimit):
                        print("Reached quota of", killLimit, "kills")
                        return
...
def killQuick(parasite, medicSkill, maelstrom, fireball, terrify):
    if not (MI.locateCenterOnScreen('images/skills/matrix_lower.png', confidence = 0.95, region = (1720, 240, 60, 70))):
        killQuicker(parasite, medicSkill, maelstrom, fireball, terrify)

    ##Fireball
    MI.click(fireball)
    ##MI.locateCenterOnScreen('images/skills/plasmafireball.png', confidence = 0.45, region = (1020,665,680,90))
...
def killQuicker(parasite, medicSkill, maelstrom, fireball, terrify):
    ##Rex
    MI.click(1690,622)
    
    ##fireball
    MI.click(fireball)
    ##MI.click(MI.locateCenterOnScreen('images/skills/plasmafireball.png', confidence = 0.45, region = (1020,665,680,90)))
...
def battleStats(kills, losses, turns, shards, newTime, startingTime, overallTime):
    ...
    print("\t\t\tPer hour battles with this time:",3600/(newTime - startingTime),"battles(s)\n")
    if(kills > 0):
