...
def Matrix():
    ...
    while(True):
        if(MI.locateCenterOnScreen('images/skills/playerbubble.png', confidence = 0.70, region = (1098, 254, 27, 20))):
            ...
            if battleOver:
                if(MI.locateCenterOnScreen('images/didYouKnow.png',confidence = 0.6, region = (1019, 752, 53, 49))):
                    time.sleep(0.5)
...
def battleStats(kills, losses, turns, shards, newTime, startingTime, overallTime):
    ...
    if kills or losses > 0:
        print("\t\t\tWin Percent:", round((kills/(kills + losses))*100,2,),"\n")
    print("\t\t\tEffective Turns:", turns,"\n")
    print("\t\t\tShards:", shards,"\n") 
    print("\t\t\tCredits:", format(shards*5000, ',d'),"\n")
    if(shards >= 1):
        print("\t\t\tShard Droprate:", round((shards/(kills + losses))*100,2),"\n")
    print("\t\t\tBattle time:",math.floor((newTime - startingTime)/60),"minute(s), and", math.floor(((newTime - startingTime)/60)%1*60), "second(s)\n")
    print("\t\t\tPer hour wins with this time:", round(3600/(newTime - startingTime),2,),"win(s)\n")
    if(kills > 0):
        print("\t\t\tWins per hour:", round(3600/((newTime - overallTime)/kills),2,),"\n")
    print("\t\t\tTime spent battling:",math.floor((newTime - overallTime)/3600), "hour(s),", math.floor(((newTime - overallTime)%3600)/60), "minute(s), and", (round(((((newTime - overallTime)%3600)/60)%1)*60,2,)), "second(s)\n\n")
